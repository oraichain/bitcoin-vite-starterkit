...
import { getAddressScriptHashesHistory, getAddressScriptHashesMempool, getTransactions, getTransaction, listUnspentAddressScriptHashes, getNewBlockHeadersSubscribe, broadcastTransaction, getFeeEstimate, relayFee } from './electrum';
...
const electrumHistoryHelper = async ({ allAddresses = [], addresses = [], changeAddresses = [], selectedCrypto = 'bitcoin' } = {}) => {
  try {
    ...
    let addressHistory = await getAddressScriptHashesHistory({
    ...
    let scriptHashMempoolResults = await getAddressScriptHashesMempool({
      addresses: allAddresses,
      id: Math.random(),
      network: networks[selectedCrypto],
      coin: selectedCrypto
    ...
    if (scriptHashMempoolResults.error === true && addressHistory.error === true)
      return {
        error: true,
    ...
    await Promise.all(
      decodedTransactions.map(async (tx) => {
        try {
          ...
          try {
            decodedTransaction = await getTransaction({
              id: Math.random(),
              txHash,
              coin: selectedCrypto
            });
          ...
          try {
            ...
            await Promise.all(
              decodedTransaction.vin.map(async (decodedInput) => {
                ...
                try {
                  ...
                  let inputDecodedTransaction = await getTransaction({
                    id: Math.random(),
            ...
            await Promise.all(
              decodedTransaction.vout.map(async (output) => {
                try {
          ...
          try {
            inputAmount = new BitcoinUnit(inputAmount, 'BTC').to('satoshi').value();
            transactionInputAmount = new BitcoinUnit(transactionInputAmount, 'BTC').to('satoshi').value();
            receivedAmount = new BitcoinUnit(receivedAmount, 'BTC').to('satoshi').value();
            outputAmount = new BitcoinUnit(outputAmount, 'BTC').to('satoshi').value();
            transactionOutputAmount = new BitcoinUnit(transactionOutputAmount, 'BTC').to('satoshi').value();
            sentAmount = new BitcoinUnit(sentAmount, 'BTC').to('satoshi').value();
...
const electrumUtxoHelper = async ({ addresses = [], changeAddresses = [], currentBlockHeight = 0, selectedCrypto = 'bitcoin' } = {}) => {
  try {
    ...
    const allUtxos = await listUnspentAddressScriptHashes({
      addresses: allAddresses,
      coin: selectedCrypto
    });

    if (allUtxos.error) return allUtxos;
    ...
    console.log(selectedCrypto, e, addresses);
    return { error: true, data: e };
  }
};

const fallbackBroadcastTransaction = async ({ rawTx = '', selectedCrypto = 'bitcoin' } = {}) => {
  const { fetchData } = require('./helpers').default;
  try {
    let config = {
      method: 'POST',
      headers: {
...
export const walletHelpers = {
  ...
  getBlockHeight: {
    electrum: async ({ selectedCrypto = 'bitcoin' } = {}) => {
      try {
        const response = await getNewBlockHeadersSubscribe({
          id: Math.random(),
          coin: selectedCrypto
        });
  ...
  relayFee: {
    electrum: async ({ selectedCrypto = 'bitcoin' } = {}) => {
      return await relayFee({ coin: selectedCrypto });
    },
    default: async ({ service = 'electrum', selectedCrypto = 'bitcoin' } = {}) => {
