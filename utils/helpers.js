import { BitcoinUnit, setDisplay } from 'bitcoin-units';
import { walletHelpers } from './walletApi';
import bitcoin, { ECPair, address as _address, payments, Psbt, crypto } from 'bitcoinjs-lib';
import { signElectrum, verify, verifyElectrum } from 'bitcoinjs-message';
 ...
 */
export const getBalanceFromUtxos = async ({ addresses = [], changeAddresses = [], selectedCrypto } = {}) => {
  try {
...
const validatePrivateKey = (privateKey = '') => {
  try {
    ...
    for (let key in networks) {
      ...
      try {
        ECPair.fromWIF(privateKey, networks[key]);
        verified = true;
        network = key;
        break;
      } catch (e) {
...
export const validateAddress = (address = '', selectedCrypto = '') => {
  try {
    //Validate address for a specific network
    if (selectedCrypto !== '') {
      const network = getCoinNetwork(selectedCrypto);
      _address.toOutputScript(address, network);
    } else {
...
export const parsePaymentRequest = (data = '') => {
...
export const getDifferenceBetweenDates = ({ start = '', end = '', time = 'minutes' } = {}) => {
  try {
...
export const getTransactionData = async ({ txId = '', selectedCrypto = 'bitcoin' } = {}) => {
  const failure = (data = '') => {
...
export const getExchangeRate = async ({ selectedCrypto = 'bitcoin', selectedCurrency = 'usd', selectedService = 'coingecko' } = {}) => {
...
export const getAddressTransactions = async ({ address = '', addresses = [], changeAddresses = [], currentBlockHeight = 0, selectedCrypto = 'bitcoin' } = {}) => {
  const failure = (data = '') => {
    return { error: true, data };
  };
...
export const getAllTransactions = async ({ allAddresses = [], addresses = [], changeAddresses = [], currentBlockHeight = 0, selectedCrypto = 'bitcoin' } = {}) => {
  const failure = (data = '') => {
    return { error: true, data };
  };
...
export const removeDupsFromArrOfObj = (arr = [], value = '') => {
  try {
...
export const getNetworkType = (selectedCrypto = 'bitcoin') => {
  try {
...
export const createTransaction = async ({
  ...
  try {
    ...
    if (message !== '') {
      ...
      if (messageLength > 0 && messageLength < lengthMin) buffers.push(Buffer.from(' '.repeat(lengthMin - messageLength), 'utf8'));
      const data = Buffer.concat(buffers);
      const embed = payments.embed({ data: [data], network });
...
export const getCoinNetwork = (coin = '') => {
  return networks[coin];
};

export const getKeyPair = ({ selectedCrypto = 'bitcoin', keyDerivationPath = '84', addressIndex = 0, mnemonic }) => {
  ...
  const root = fromSeed(mnemonicToSeedSync(mnemonic), network);
  const addressPath = `m/${keyDerivationPath}'/${coinTypePath}'/0'/0/${addressIndex}`;
  const keyPair = root.derivePath(addressPath);
  return keyPair;
};

export const generateAddresses = async ({ addressAmount = 0, changeAddressAmount = 0, mnemonic, addressIndex = 0, changeAddressIndex = 0, selectedCrypto = 'bitcoin', keyDerivationPath = '84', addressType = 'bech32' } = {}) => {
  const failure = (data) => {
    return { error: true, data };
  };
  try {
...
export const getAddress = (keyPair, network, type = 'bech32') => {
  try {
...
export const openTxId = (txid = '', selectedCrypto = '') => {
...
export const pauseExecution = (duration = 500) => {
...
export const vibrate = (type = 'impactHeavy') => {
  try {
    if (type === 'default') {
...
export const getInfoFromAddressPath = async (path = '') => {
  try {
...
export const removeDecimals = (str) => {
  return str.replace(/^([^.]*\.)(.*)$/, function (a, b, c) {
    return b + c.replace(/\./g, '');
  });
};

export const removeAllButFirstInstanceOfPeriod = (s) => {
  try {
...
export const decodeOpReturnMessage = (opReturn = '') => {
  try {
...
export const signMessage = async ({ message = '', addressType = 'bech32', path = "m/84'/0'/0'/0/0", mnemonic, selectedCrypto = 'bitcoin' } = {}) => {
  try {
    ...
    let signature = '';
    if (addressType === 'legacy') {
      signature = signElectrum(message, privateKey, keyPair, messagePrefix);
    } else {
      signature = signElectrum(message, privateKey, keyPair.compressed, messagePrefix, sigOptions);
    }
...
export const verifyMessage = ({ message = '', address = '', signature = '', selectedCrypto = '' } = {}) => {
  try {
    const network = networks[selectedCrypto];
    const messagePrefix = network.messagePrefix;
    let isValid = false;
    try {
      isValid = verify(message, address, signature, messagePrefix);
    } catch (e) {}
    //This is a fix for https://github.com/bitcoinjs/bitcoinjs-message/issues/20
    if (!isValid) isValid = verifyElectrum(message, address, signature, messagePrefix);
    return isValid;
  } catch (e) {
    console.log(e);
    return false;
...
export const getBaseDerivationPath = ({ keyDerivationPath = '84', selectedCrypto = 'bitcoin' }) => {
  try {
...
export const loginWithBitid = async ({ url = '', addressType = 'bech32', keyDerivationPath, selectedCrypto, selectedWallet } = {}) => {
  try {
...
export const sortArrOfObjByKey = (arr = [], key = '', ascending = true) => {
  try {
...
export const getFiatBalance = ({ balance = 0, exchangeRate = 0 } = {}) => {
  try {
    setDisplay('usd', exchangeRate);
    const fiatBalance = new BitcoinUnit(balance, 'satoshi').to('usd').value().toFixed(2);
...
export const cryptoToFiat = ({ amount = 0, exchangeRate = 0 } = {}) => {
  try {
    amount = Number(amount);
    setDisplay('usd', exchangeRate);
    return new BitcoinUnit(amount, 'satoshi').to('usd').value().toFixed(2);
  } catch (e) {
    console.log(e);
  }
};

export const satsToBtc = ({ amount = 0 } = {}) => {
  try {
    amount = Number(amount);
    return new BitcoinUnit(amount, 'satoshi').to('BTC').value();
  } catch (e) {
    return amount;
  }
};
...
export const fiatToCrypto = ({ amount = 0, exchangeRate = 0 } = {}) => {
  try {
    amount = Number(amount);
    setDisplay('usd', exchangeRate);
    return new BitcoinUnit(amount, 'usd').to('satoshi').value().toFixed(0);
  } catch (e) {
    console.log(e);
  }
};

export const getLastWordInString = (phrase = '') => {
  try {
...
*/
export const getByteCount = (inputs, outputs, message = '') => {
  try {
    let totalWeight = 0;
...
export const getScriptHash = (address = '', network = networks['bitcoin']) => {
  if (typeof network === 'string' && network in networks) network = networks[network];
  const script = _address.toOutputScript(address, network);
  let hash = crypto.sha256(script);
  const reversedHash = Buffer.from(hash.reverse());
  return reversedHash.toString('hex');
};

export const wallet = walletHelpers;
