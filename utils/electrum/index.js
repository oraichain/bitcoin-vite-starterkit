...
export const batchAddresses = ({ coin = 'bitcoinTestnet', scriptHashes = [] } = {}) => {
  return new Promise(async (resolve) => {
    const response = await promiseTimeout(getTimeout(scriptHashes), clients.mainClient[coin].blockchain_scripthash_getHistoryBatch(scriptHashes));
    resolve(response);
  });
};
  ...
  return new Promise(async (resolve) => {
    try {
      ...
      if (needToConnect) {
         ...
         */
        const scriptHash = 'bc2205241e0e7906f802e02e538b75a15c3cade86d05b3235ce8be92254db83a';
        const testResponses = await Promise.all([pingServer(), getAddressScriptHashesBalance({ coin, addresses: [{ scriptHash }] })]);
...
export const subscribeAddress = async ({ id = 'wallet0bitcoin', address = '', coin = 'bitcoin', onReceive = (data) => console.log(data) } = {}) => {
  try {
    if (clients.mainClient[coin] === false) await connectToRandomPeer(coin, clients.peers[coin]);
...
export const getAddressScriptHashBalance = ({ scriptHash = '', id = Math.random(), coin = '' } = {}) => {
  ...
  return new Promise(async (resolve) => {
    try {
      if (clients.mainClient[coin] === false) await connectToRandomPeer(coin, clients.peers[coin]);
      const { error, data } = await promiseTimeout(getTimeout(), clients.mainClient[coin].blockchain_scripthash_getBalance(scriptHash));
...
export const getAddressScriptHashesBalance = ({ addresses = [], id = Math.random(), coin = '' } = {}) => {
  ...
  return new Promise(async (resolve) => {
    try {
      ...
      const { error, data } = await promiseTimeout(timeout, clients.mainClient[coin].blockchain_scripthashes_getBalance(addresses.map((a) => a.scriptHash)));
...
export const getAddressScriptHashHistory = async ({ scriptHash = '', id = Math.random(), coin = '' } = {}) => {
  ...
  return new Promise(async (resolve) => {
    try {
      if (clients.mainClient[coin] === false) await connectToRandomPeer(coin, clients.peers[coin]);
      const { error, data } = await promiseTimeout(getTimeout(), clients.mainClient[coin].blockchain_scripthash_getHistory(scriptHash));
      resolve({ id, error, method, data, coin });
    } catch (e) {
      console.log(e);
      resolve({ id, error: true, method, data: e, coin });
...
export const getAddressScriptHashesHistory = ({ addresses = [], id = Math.random(), coin = '' } = {}) => {
  ...
  return new Promise(async (resolve) => {
    try {
      if (clients.mainClient[coin] === false) await connectToRandomPeer(coin, clients.peers[coin]);
      const timeout = getTimeout({ arr: addresses });
      const { error, data } = await promiseTimeout(timeout, clients.mainClient[coin].blockchain_scripthashes_getHistory(addresses.map((a) => a.scriptHash)));
      resolve({ id, error, method, data, coin });
    } catch (e) {
      console.log(e);
...
export const listUnspentAddressScriptHash = ({ scriptHash = '', id = Math.random(), coin = '' } = {}) => {
  ...
  return new Promise(async (resolve) => {
    try {
      if (clients.mainClient[coin] === false) await connectToRandomPeer(coin, clients.peers[coin]);
      const { error, data } = await promiseTimeout(getTimeout(), clients.mainClient[coin].blockchain_scripthash_listunspent(scriptHash));
      resolve({ id, error, method, data, coin });
    } catch (e) {
      console.log(e);
      resolve({ id, error: true, method, data: e, coin });
...
export const listUnspentAddressScriptHashes = async ({ addresses = [], id = Math.random(), coin = '' } = {}) => {
  const method = 'listUnspentAddressScriptHashes';
  if (clients.mainClient[coin] === false) await connectToRandomPeer(coin, clients.peers[coin]);
  try {
    const timeout = getTimeout({ arr: addresses });
    const { error, data } = await promiseTimeout(timeout, clients.mainClient[coin].blockchain_scripthashes_listunspent(addresses.map((a) => a.scriptHash)));
...
export const getAddressScriptHashMempool = ({ scriptHash = '', id = Math.random(), coin = '' } = {}) => {
  ...
  return new Promise(async (resolve) => {
    try {
      if (clients.mainClient[coin] === false) await connectToRandomPeer(coin, clients.peers[coin]);
      const { error, data } = await promiseTimeout(getTimeout(), clients.mainClient[coin].blockchain_scripthash_getMempool(scriptHash));
      resolve({ id, error, method, data, coin });
    } catch (e) {
      console.log(e);
      resolve({ id, error: true, method, data: [], coin });
...
export const getAddressScriptHashesMempool = ({ addresses = [], id = Math.random(), coin = '' } = {}) => {
  ...
  return new Promise(async (resolve) => {
    try {
      if (clients.mainClient[coin] === false) await connectToRandomPeer(coin, clients.peers[coin]);
      const timeout = getTimeout({ arr: addresses });
      const { error, data } = await promiseTimeout(timeout, clients.mainClient[coin].blockchain_scripthashes_getMempool(addresses.map((a) => a.scriptHash)));
      resolve({ id, error, method, data, coin });
    } catch (e) {
      console.log(e);
      resolve({ id, error: true, method, data: e, coin });
